...
5. **封装规则**: 同一操作在 5 个以上用例中重复时才允许封装进基础类，避免过度封装；低频动作保留在具体用例中

---

## Healer 调试更新规则

- **必须同步计划**：修复导致步骤表达、expect 描述、应用文案、断言目标、交互路径或用例注释变化时，必须同步对应 `aaa_*.md`。
- **无需同步计划**：仅调整选择器实现、等待方式、timeout、代码结构或可维护性，且测试计划功能语义没有变化时，不需要更新计划。
- **计划定位规则**：优先读取 spec 顶部 `// spec:` 指向的计划文件；没有时使用 spec 同目录下唯一的 `aaa_*.md`。
- **计划更新方式**：不直接覆盖原用例；在原用例下新增或替换同名 `[UPDATED]` 记录，保留最新验证通过的步骤。
- **审批语义**：`[UPDATED]` 记录代表待用户审查合并；不要把它当成已合并原计划。
- **最终说明**：healer 收尾时必须说明修改了哪些 spec、同步了哪些 plan；未同步时必须说明原因。
