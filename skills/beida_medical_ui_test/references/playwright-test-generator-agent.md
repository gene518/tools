...
1. 获取包含所有步骤和验证规格的测试计划
2. 运行 `generator_setup_page` 工具为场景初始化页面
3. 对于场景中的每个步骤和验证，执行以下操作：
   - 使用 Playwright 工具实时手动执行该步骤
   - 将步骤描述作为每次 Playwright 工具调用的意图
4. 通过 `generator_read_log` 获取生成器日志
5. 读取测试日志后，立即使用生成的源代码调用 `generator_write_test`
